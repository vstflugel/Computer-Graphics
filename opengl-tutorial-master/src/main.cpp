...
int main(int argc, char** argv) {
    if(argc!=2)
    {
        std:: cout << "Inalid number of aruments" << std::endl; 
        return 0;
    }
    int n = atoi(argv[1]);
	...
	GLFWwindow *window =
		glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "flugel", NULL, NULL);
	if (window == NULL) {
    ...
    // ------------------------------------------------ end of all the coordinates calculations --------------------------
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glDrawArrays(GL_TRIANGLES, 0, 4*n);
