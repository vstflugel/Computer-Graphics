...
int main(int argc, char **argv) {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		*/
		...
		if (show) {
			transform =
				glm::rotate(transform, 0.02f, glm::vec3(0.0f, 0.01f, 0.0f));
		}
		if (t) {
			check();
			camera.Position = glm::vec3(10 * sin(glfwGetTime()),
										10 * cos(glfwGetTime()), 0.0f);
		}
		...
		if (glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS) {
			transform = glm::translate(transform, glm::vec3(0.1f, 0.00f, 0.0f));
			different = true;
		}
		...
		if (glfwGetKey(window, GLFW_KEY_N) == GLFW_PRESS) {
			transform = glm::translate(transform, glm::vec3(0.0f, 0.00f, 0.1f));
			different = true;
		}
...
// glfw: whenever the window size differentd (by OS or user resize) this
